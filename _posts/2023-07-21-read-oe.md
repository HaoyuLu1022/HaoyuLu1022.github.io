...
, i.e., the OE posterior. When sequence Eq. (17) approaches equilibrium, the probability that it is at state 
...
wherein mean $\mu_k\in [0, 1]$ determines the most possible position of event $k$ on its LOR $j_k$, and standard deviation $\sigma_k$ is decided by both the length of LOR $j_k$ and TOF estimation accuracy of the scanner. Specifically, $\mu_k$ and $\sigma_k$ can be pre-calculated by the distance between 
$\boldsymbol{x}_j^{(1)}$
 and 
$\boldsymbol{x}_j^{(2)}$
, and the arrival time difference of the annihilation photons. Note that since $\boldsymbol{\gamma}_{j_k}$ is injective, $\boldsymbol{\gamma}_{j_k}(t)$ can be mapped to $t\in [0, 1]$.
