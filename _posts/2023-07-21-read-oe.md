...
There are 3 registered events $k=1, 2, 3$, wherein event $k=1$ is detected by pair $j=1$, while events $k=2, 3$ are detected by $j=2$. Then the incomplete data acquired is $\boldsymbol{n}^*=\left\{n_1^*=1, n_2^*=2, n_3^*=\cdots=n_6^*=0\right\}$. To simplify, perfect collinearity is assumed, i.e., $\alpha_{15}=\alpha_{26}=\alpha_{31}=\alpha_{43}=0$.
...
that works for all $S_0\in \mathbb{N}_0$ and all $i$. We regard the left half of the above Eq, (18) as the approximation of MMSE estimation MMSE估计$E[n_i|\mathbf{n^{\ast}}]$.
