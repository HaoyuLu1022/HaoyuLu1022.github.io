...
$$
\mathcal{P}\left(\boldsymbol{n} \middle| \boldsymbol{n}^*\right)=\frac{\mathcal{P}\left(\boldsymbol{n}^* \middle| \boldsymbol{n}\right) \mathcal{P}(\boldsymbol{n})}{\mathcal{P}\left(\boldsymbol{n}^*\right)}=\frac{\mathcal{P}\left(\boldsymbol{n}^* \middle| \boldsymbol{n}\right)}{\mathcal{P}\left(\boldsymbol{n}^*\right)} \int_{\boldsymbol{\lambda}} \mathcal{P}_{ \boldsymbol{\lambda}}(\boldsymbol{n}) \mathrm{d} \mathcal{P}(\boldsymbol{\lambda}), \tag{10}
$$

- $\mathcal{P}\left(\boldsymbol{n}^* \middle| \boldsymbol{n}\right)$ is the probability to observe the acquired data $\mathbf{n^{\ast}}$ given complete data $\mathbf{n}$
...
$$
\mathcal{P}\left(\boldsymbol{n} \middle| \boldsymbol{n}^*\right) \propto \varphi(\boldsymbol{n}) \int_{\boldsymbol{\lambda}} \psi(\boldsymbol{\lambda}, \boldsymbol{n}) \mathrm{d} \mathcal{P}(\boldsymbol{\lambda}). \tag{12}
$$

For simplification, a flat prior is used for $\mathcal{P}(\boldsymbol{\lambda})$; considering Gamma distribution, here we have 

$$
\mathcal{P}_{\mathrm{f}}\left(\boldsymbol{n} \middle| \boldsymbol{n}^*\right) \propto \varphi(\boldsymbol{n}) \prod_i \frac{n_{i} !}{\varepsilon_i^{n_i}}. \tag{13}
$$
...
$$
\mathcal{P}_{\mathrm{f}}\left(\boldsymbol{\omega} \middle| \boldsymbol{n}^*\right) \propto \prod_i \frac{n_{i} !}{\varepsilon_i^{n_i}} \prod_j \alpha_{i j}^{n_{i j}}. \tag{16}
$$
...
Let the equilibrium distribution be $\mathcal{P}(\cdot\middle|\boldsymbol{n}^*)$, i.e., the OE posterior. When sequence Eq. (17) approaches equilibrium, the probability that it is at state $\omega\in \Omega$ is approximately $\mathcal{P}\left(\boldsymbol{\omega} \middle| \boldsymbol{n}^*\right)$. In equilibrium, the states $\omega_s, s \in \mathbb{N}_0$, can be treated as independent samples from the OE posterior. In combination, this yields

$$
\frac{1}{S-S_0} \sum_{s=S_0}^S n_i\left(\boldsymbol{\omega}_s\right) \underset{S \rightarrow \infty}{\longrightarrow} \mathbb{E}\left[n_i \middle| \boldsymbol{n}^*\right], \tag{18}
$$
...
$$
Q(\cdot \middle| \boldsymbol{\omega}): \Omega \rightarrow[0,1], \quad \boldsymbol{\omega}^{\prime} \mapsto Q\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{\omega}\right). \tag{19}
$$

Proposed new state is accepted with an acceptance probability

$$
A(\omega^{\prime}\middle|\omega)\in [0, 1]. \tag{20}
$$
...
$$
A\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{\omega}\right)=\min \left\{1, \frac{\mathcal{P}\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{n}^*\right)}{\mathcal{P}\left(\boldsymbol{\omega} \middle| \boldsymbol{n}^*\right)} \cdot \frac{Q\left(\boldsymbol{\omega} \middle| \boldsymbol{\omega}^{\prime}\right)}{Q\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{\omega}\right)}\right\}, \tag{21}
$$

wherein $Q\left(\boldsymbol{\omega} \middle| \boldsymbol{\omega}^{\prime}\right)$ is the probability to propose a transition to state $\omega$ when the Markov chain is in state $\omega^{\prime}$, vice versa for $Q\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{\omega}\right)$. 
...
$$
\frac{\mathcal{P}_{\mathrm{f}}\left(\boldsymbol{\omega}^{\prime} \middle| \boldsymbol{n}^*\right)}{\mathcal{P}_{\mathrm{f}}\left(\boldsymbol{\omega} \middle| \boldsymbol{n}^*\right)}=\frac{\alpha_{i^{\prime} j_k}}{\alpha_{i j_k}} \cdot \frac{\varepsilon_i}{\varepsilon_{i^{\prime}}} \cdot \frac{n_{i^{\prime}}(\boldsymbol{\omega})+1}{n_i(\boldsymbol{\omega})}. \tag{22}
$$
